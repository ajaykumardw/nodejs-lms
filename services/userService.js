...
const importUsers = async (res, userId, chunk, roleIds = []) => {
  try {
    ...
    const existingEmailHashes = new Set(
      existingEmailUsers.map(u => u.email_hash)
    )

    const phones = chunk
      .map(u => normalizePhone(String(u.PhoneNo || '')))
      .filter(Boolean)
    ...
    const existingPhoneHashes = new Set(
      existingPhoneUsers.map(u => u.phone_hash)
    )
...
const getUserStats = async userId => {
  try {
    const [total, active, inactive, notLoggedIn] = await Promise.all([
      User.countDocuments({
        company_id: userId
      }),
      ...
      User.countDocuments({
        company_id: userId,
        $or: [{ last_login: { $exists: false } }, { last_login: null }]
      })
    ])
    ...
    return {
      ...
      not_logged_in_users: notLoggedIn
    }
  } catch (error) {
