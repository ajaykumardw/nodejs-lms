...
const pick = (obj, fields) =>
  Object.fromEntries(fields.map(key => [key, obj[key]]))
...
exports.createUserAPI = async (req, res, next) => {
  try {
    ...
    await replaceTemplateField({
      ...
      means: 'User Registration',
      explanation: 'User registration successfully',
...
const processEmployeeCodesForUser = async ({
  ...
  for (const code of normalizedCodes) {
    if (!existingCodesMap.has(code)) {
      ...
      const index = updatedExistingCodes.findIndex(
...
exports.updateUserAPI = async (req, res, next) => {
  try {
    ...
    return successResponse(
      res,
      `${updatedUser.first_name} account changes saved!`,
      updatedUser
    )
  } catch (error) {
    next(error)
...
exports.markActiveUserCodeAPI = async (req, res, next) => {
  try {
    ...
    return successResponse(
      res,
      `Employee ID changed for ${existingUser.first_name}`,
...
exports.updateStatusAPI = async (req, res, next) => {
  try {
    ...
    return successResponse(
      res,
      `${user.first_name} account status marked as ${
