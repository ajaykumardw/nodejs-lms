...
exports.getFetchActivity = async (req, res, next) => {
    try {
        ...
        const activities = await Activity.aggregate([
            {
                $match: {
                    _id: mongoose.Types.ObjectId.createFromHexString(activityId),
...
exports.postNewAttempController = async (req, res, next) => {
    try {

        const userId = req?.userId

        const { activityId, moduleId, contentFolderId, moduleTypeId, batchId, sessionId } = req?.body
                                    ...
                                    branches: [
                                        {
                                            case: {
                                                $eq: [
                                                    ...
                                                    mongoose.Types.ObjectId.createFromHexString('688723af5dd97f4ccae68834')
                                                ]
                                            },
                                            then: {
                                                $gt: [
                                        ...
                                        },
                                        {
                                            case: {
                                                $eq: [
                                                    '$$activity.module_type_id', mongoose.Types.ObjectId.createFromHexString('688723af5dd97f4ccae68835')
                                                ]
                                            },
                                            then: {
                                                $gt: [
                                        ...
                                        },
                                        {
                                            case: {
                                                $eq: [
                                                    '$$activity.module_type_id', mongoose.Types.ObjectId.createFromHexString('688723af5dd97f4ccae68836')
                                                ]
                                            },
                                            then: {
                                                $gt: [
                                        ...
                                        },
                                        {
                                            case: {
                                                $eq: [
                                                    '$$activity.module_type_id', mongoose.Types.ObjectId.createFromHexString('688723af5dd97f4ccae68837')
                                                ]
                                            },
                                            then: {
                                                $gt: [
                                        ...
                                        },
                                        {
                                            case: {
                                                $eq: [
                                                    '$$activity.module_type_id',
                                                    mongoose.Types.ObjectId.createFromHexString('68886902954c4d9dc7a379bd')
                                                ]
                                            },
                                            then: {
                                                $gt: [
        ...
        const isScheduleBefore = relativeEndDate && new Date(relativeEndDate).getTime() > Date.now()

        const isPreCompleted = pre_activities.length > 0 && pre_activities.every(activity => pre_logs.some(log => log.activity_id.toString() === activity._id.toString() && log.progress_status === '3'))
        ...
        const finalActivityCompletion = !pre_activity_report && final_activity_report
...
exports.postScormData = async (req, res, next) => {
    try {
                                ...
                                $switch: {
                                    branches: [
                                        // Document
                                        {
                                            case: {
                                                $eq: [
                                                    '$$activity.module_type_id', mongoose.Types.ObjectId.createFromHexString('688723af5dd97f4ccae68834')
                                                ]
                                            },
                                            then: {
                                                $gt: [
                                        ...
                                        // Quiz
                                        {
                                            ...
                                            then: {
                                                $gt: [
                                                    {
                                                        $size: { $ifNull: ['$$activity.questions', []] }
                                                    },
                                                    0
                                                ]
                                            }
        ...
        const isFinalCompleted = parsed?.lessonStatus == 'passed' || parsed?.lessonStatus == 'completed'

        const isPassed = parsed?.lessonStatus == 'passed' || parsed?.lessonStatus == 'completed'

        const passPercent = parsed?.lessonStatus == 'passed' || parsed?.lessonStatus == 'completed' ? '100' : '0'
        ...
        const isCompleted = activities.length > 0 && activities.every(activity =>
            logs.some(
                log =>
                    log.activity_id.toString() === activity._id.toString() &&
                    log.progress_status === '3'
        ...
        const finalData = {
            ...
            is_survey_completed: (!userSurvey && moduleSetting?.feedbackSurveyEnabled) || false,
...
exports.postReportController = async (req, res, next) => {
    try {
                                ...
                                $switch: {
                                    branches: [
                                        ...
                                        },
                                        {
                                            case: {
                                                $eq: [
                                                    '$$activity.module_type_id', mongoose.Types.ObjectId.createFromHexString('68886902954c4d9dc7a379bd')
                                                ]
                                            },
                                            then: {
                                                $gt: [
        ...
        if (moduleTypeId == '688723af5dd97f4ccae68834') {
            ...
            perComplete = totalVideoTime > 0 ? (roundedViewed / Number(totalVideoTime)) * 100 : 0
...
exports.postEndAttemptController = async (req, res, next) => {
    try {
        const {
            moduleId,
            contentFolderId,
