...
exports.getContentFolderAPI = async (req, res, next) => {
  try {
    ...
    const totalItems = await ContentFolder.countDocuments({
      created_by: userId,
      program_id: id
    })

    const contentFolder = await ContentFolder.find({
      created_by: userId,
      program_id: id
    })
      .skip(skip)
...
exports.postContentFolderAPI = async (req, res, next) => {
  try {
    ...
    const { title, description, checkCertificate, certificateId } = req.body
...
exports.editContentFolderAPI = async (req, res, next) => {
  try {
    ...
    return successResponse(
      res,
      'Content folder fetched successfully',
      contentFolder
    )
  } catch (error) {
    next(error)
...
exports.updateContentFolderAPI = async (req, res, next) => {
  try {
    ...
    const selectContentFolder = await ContentFolder.findOne({
      created_by: userId,
      _id: cid,
      program_id: id
    })
    ...
    await ContentFolder.findOneAndUpdate(
      { created_by: userId, _id: cid, program_id: id },
      {
        $set: {
          title,
          description,
