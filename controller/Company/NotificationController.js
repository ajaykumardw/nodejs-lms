...
exports.putUpdateNotificationAPI = async (req, res, next) => {
  try {
    ...
    // save file paths if uploaded
    if (req.files?.header_logo) {
      updateData.header_logo = req.files.header_logo[0].path
    }

    if (req.files?.footer_logo) {
      updateData.footer_logo = req.files.footer_logo[0].path
    }
    ...
    const Notification = await notification.aggregate([
      ...
      },
      {
        $addFields: {
          ...
          show_footer_logo: {
            $ifNull: [
              '$matched_user_input.show_footer_logo',
              '$show_footer_logo'
            ]
          },
          header_logo: {
          ...
          header_logo_align: {
            $ifNull: [
              '$matched_user_input.header_logo_align',
              '$header_logo_align'
            ]
          },
          footer_logo_align: {
            $ifNull: [
              '$matched_user_input.footer_logo_align',
              '$footer_logo_align'
            ]
          }
        }
...
exports.updateNotificationAPI = async (req, res, next) => {
  try {
    ...
    if (req.files?.header_logo) {
      ...
      existNotification &&
      existNotification?.user_input?.[0]?.header_logo
    ) {
    ...
    if (req.files?.footer_logo) {
      ...
      existNotification &&
      existNotification?.user_input?.[0]?.footer_logo
    ) {
    ...
    if (existNotification) {
      Notification = await notification.updateOne(
        { _id: id, 'user_input.created_by': userId },
        {
          $set: {
...
exports.getCheckSelectNotificationAPI = async (req, res, next) => {
  try {
    ...
    if (existNotification) {
      // Update existing user_input record
      await notification.updateOne(
        ...
        },
        {
          $set: {
            ...
            'user_input.$.header_logo_align': header_logo_align,
            'user_input.$.show_footer_logo': show_footer_logo,
            'user_input.$.header_logo': header_logo,
            'user_input.$.footer_logo': footer_logo,
            'user_input.$.footer_logo_align': footer_logo_align
          }
        }
      )
    } else {
      // Create new user_input record
      await notification.updateOne(
        { _id: id },
        {
          $push: {
