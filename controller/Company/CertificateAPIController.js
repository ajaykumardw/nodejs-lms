...
exports.getCertificateSettings = async (req, res, next) => {
  try {
    ...
    const certificateSettings = settingConfig
      ? settingConfig.certificate_setting_data_id
      : '6a153d4a393b1c736064377b'

    const appConfig = await AppConfig.findOne({ type: 'certificate_setting' })

    if (!appConfig) {
      return errorResponse(res, 'Certificate settings not found', {}, 404)
    }

    const finalData = {
      certificate_setting_data: appConfig.certificate_setting_data,
      certificateSettingId: certificateSettings
    }
...
exports.postCertificateSettingAPI = async (req, res, next) => {
  try {
    const userId = req?.userId

    const { certificateSettingId } = req?.body
    ...
    if (settingConfig) {
      settingConfig.certificate_setting_data_id = certificateSettingId
      settingConfig.updated_by = userId
      settingConfig.updated_at = new Date()

      await settingConfig.save()
    } else {
      const newSettingConfig = new SettingConfig({
        type: 'certificate_setting',
        certificate_setting_data_id: certificateSettingId,
        created_by: userId,
        created_at: new Date()
      })

      await newSettingConfig.save()
    }

    return successResponse(res, 'Certificate setting updated successfully')
  } catch (error) {
    next(error)
  }
}
