    ...
    const report = await Module.aggregate([
      ...
      },
      {
        $addFields: {
          ...
          inProgressPercent: {
            $multiply: [
              { $divide: ['$inProgressModules', '$totalModules'] },
              100
            ]
          },
          notStartedPercent: {
            $multiply: [
              { $divide: ['$notStartedModules', '$totalModules'] },
              100
            ]
          }
        }
          ...
          completedModules: {
            $size: {
              $filter: {
                input: '$modules',
                as: 'm',
                cond: { $eq: ['$$m.moduleStatus', 'completed'] }
              }
            }
          },
          inProgressModules: {
            $size: {
              $filter: {
                input: '$modules',
                as: 'm',
                cond: { $eq: ['$$m.moduleStatus', 'in_progress'] }
              }
            }
          },
          notStartedModules: {
            $size: {
              $filter: {
                input: '$modules',
                as: 'm',
                cond: { $eq: ['$$m.moduleStatus', 'not_started'] }
              }
            }
          }
        }
...
exports.getByLearnerController = async (req, res, next) => {
  try {
    ...
    if (data.designation) {
      matchQuery.designation_id = mongoose.Types.ObjectId.createFromHexString(
        data.designation
      )
    }

    if (data.department) {
      matchQuery.zone_id = mongoose.Types.ObjectId.createFromHexString(
        data.department
      )
    }
    ...
    if (data.region) {
      matchQuery.region_id = mongoose.Types.ObjectId.createFromHexString(
        data.region
      )
    }
...
exports.getFilterDataController = async (req, res, next) => {
  try {
    ...
    const participationType = await ParticipationType.find({
      company_id: userId
    })
...
exports.getModuleTypeUserData = async (req, res, next) => {
  try {
    ...
    return successResponse(
      res,
      'Allowed modules fetched successfully',
      finalData
    )
  } catch (error) {
    next(error)
...
exports.getQuizAssessmentReportController = async (req, res, next) => {
  try {
    ...
    const modTypId = mongoose.Types.ObjectId.createFromHexString(
...
exports.getScormDetailReportDataController = async (req, res, next) => {
  try {
    ...
    if (data.contentFolder) {
      matchQuery.content_folder_id =
        mongoose.Types.ObjectId.createFromHexString(data.contentFolder)
    }

    if (data.module) {
      matchQuery.module_id = mongoose.Types.ObjectId.createFromHexString(
        data.module
      )
    }

    if (data.program) {
      matchQuery.program_id = mongoose.Types.ObjectId.createFromHexString(
        data.program
      )
    }

    if (data.user) {
      matchQuery.user_id = mongoose.Types.ObjectId.createFromHexString(
        data.user
      )
    }

    if (data.completedStatus && data.completedStatus === '1') {
      matchQuery.is_completed = true
    }

    if (data.passedStatus && data.passedStatus === '1') {
      matchQuery.is_passed = true
    }
    ...
    const finalActivity = activityReport.map(item => ({
      ...
      user_info: {
        ...
        email: item.user_info?.email ? decrypt(item.user_info.email) : null,

        phone: item.user_info?.phone ? decrypt(item.user_info.phone) : null
      }
    }))
        ...
        ? [
            {
              $match: {
                'roleUser.role_id':
                  mongoose.Types.ObjectId.createFromHexString(role)
              }
            }
          ]
        : []),
...
exports.getUserReportController = async (req, res, next) => {
  try {
    ...
    // DEPARTMENT FILTER
    if (department && department !== 'null') {
      matchQuery.department_id =
        mongoose.Types.ObjectId.createFromHexString(department)
    }

    // DESIGNATION FILTER
    if (designation && designation !== 'null') {
      matchQuery.designation_id =
        mongoose.Types.ObjectId.createFromHexString(designation)
    }

    // PARTICIPATION TYPE FILTER
    if (participationType && participationType !== 'null') {
      matchQuery.participation_type_id =
        mongoose.Types.ObjectId.createFromHexString(participationType)
    }
    ...
    return successResponse(res, 'User data fetched successfully', finalData)
  } catch (error) {
    next(error)
  }
}
        ...
        $addFields: {
          ...
          completionStatus: {
            $switch: {
              branches: [
                {
                  case: { $eq: ['$progress_status', '1'] },
                  then: 'Not started'
                },
                {
                  case: { $eq: ['$progress_status', '2'] },
                  then: 'In progress'
                },
            ...
            $cond: [
              ...
              },
              {
                $dateToString: {
                  ...
                  date: {
                    $toDate: {
                      $subtract: ['$end_activity_time', '$start_activity_time']
                    }
                  }
                }
              },
...
const hasAllValues = (value, allValues) => {
  ...
  return (
    arr.length === allValues.length && allValues.every(v => arr.includes(v))
  )
}
...
exports.getMiscellaneousReportController = async (req, res, next) => {
  try {
    ...
    if (
      completionArr.length &&
      !hasAllValues(data.completionStatus, ['1', '2', '3'])
    ) {
